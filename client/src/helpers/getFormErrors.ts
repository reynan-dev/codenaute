...
export const getFormErrors = (fieldsValue: FieldsValue) => {
	...
	if (username && !username.match(USERNAME_REGEX)) errorMessages.username = USERNAME_MUST_BE_LONG_ERROR_MESSAGE;
	if (email && !email.match(EMAIL_REGEX)) errorMessages.email = INVALID_EMAIL_ERROR_MESSAGE;
	if (password && !password.match(PASSWORD_REGEX)) errorMessages.password = PASSWORD_FORMAT_ERROR_MESSAGE;
